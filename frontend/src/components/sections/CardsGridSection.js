  ...
  return (
    <section className={`py-20 md:py-28 ${hasIcons ? 'bg-[#f7f5f2]/60' : ''}`}>
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <div className={`grid grid-cols-1 ${useLargeStyle ? 'md:grid-cols-2' : items.length <= 3 ? 'sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3' : 'sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4'} gap-6`}>
          {items.map((item, i) => {
            const slug = (item.title || 'categories').toLowerCase().replace(/ & /g, '-').replace(/ /g, '-').replace(/[^\w-]+/g, '');
            return (
              <Link 
